...
const calculateWeaknesses = (types) => {
  ...
  types.forEach((type) => {
    const typeLowerCase = type.toLowerCase(); // Convert về chữ thường
    console.log("Type:", typeLowerCase);

    if (weaknesses[typeLowerCase]) {
      weaknesses[typeLowerCase].weak.forEach((t) => total[t]++);
      weaknesses[typeLowerCase].resistant.forEach((t) => total[t]--);
      weaknesses[typeLowerCase].nullified.forEach((t) => total[t]--);
    } else {
      console.warn(
        `Warning: Type ${typeLowerCase} không tồn tại trong weaknesses`
      );
    }
  });
